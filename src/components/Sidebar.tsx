...
export function Sidebar({ className, onItemClick, isMobile }: SidebarProps) {
  ...
  const allMenuItems = [
    ...
    // Aşağıdaki sayfalar henüz hazır değil - gizlendi (direct URL erişimi hala korunuyor)
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        collapsed ? "justify-center" : (isMobile ? "justify-start" : "justify-between")
      )}>
        {!collapsed && (
        ...
        {!isMobile && (
          <Button
            ...
            className={cn(
              "h-8 w-8 text-gray-600 hover:bg-gray-100 rounded-lg cursor-pointer"
            )}
          >
      ...
      <nav className="flex-1 p-3 overflow-y-auto">
        <div className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={onItemClick} className="cursor-pointer">
                <Button
                  variant="ghost"
                  className={cn(
                    "w-full justify-start gap-3 h-11 text-gray-700 rounded-lg transition-all font-medium cursor-pointer",
      ...
      {!collapsed && (
        <div className="p-4 border-t border-gray-100">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="w-full justify-start gap-2 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
          >
      ...
      {/* Collapsed User Avatar - Removed for cleaner look */}
    </div>
  )
}
